                                ...
                                <td class="align-middle">
                                    @if ($payment->status !== 'disetujui' && $payment->status !== 'ditolak')
                                        <form action="{{ route('admin.payment.approve', $payment->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="btn" style="background-color: #c39a74; color: white">
                                                Setujui
                                            </button>
                                        </form>
                                        ...
                                        <form action="{{ route('admin.payment.reject', $payment->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="btn" style="background-color: #c39a74; color: black">
                                                Tolak
                                            </button>
                                        </form>
                                    @else
                                        <button class="btn" style="background-color: #e3ded1; color: white" disabled>
                                            Setujui
                                        </button>
                                        <button class="btn" style="background-color: #c1b59f; color: black" disabled>
                                            Tolak
                                        </button>
                                    @endif
                                    ...
                                    <form onclick="return confirm('Apakah anda yakin ingin menghapus payment ini?');" class="d-inline" action="{{ route('payments.destroy', $payment->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn"  style="background-color: #908471; color: white">Returned</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
